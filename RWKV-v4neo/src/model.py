...
class BlockStateList:
    ...
    def create(N, B, C, device, dtype, att_channels):
        ...
        for i in range(att_channels):
            ...
            result.att_shift_channel_states[i][:] = 0
    ...
    def forward(self, x, last_state: BlockState):
        ...
        for i in range(self.att_channels):
            att_out, att_state = self.att[i](
                x_ln1,
                last_state.time_mix_states[i] if last_state.time_mix_states is not None and i in last_state.time_mix_states else TimeMixState(None, None),
            )
